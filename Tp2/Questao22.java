...
class Lista {
   ...
   private int n,numComparacoes,numMovimentacoes;
  ...
  /**
   ...
   public void mostrarRec(int i){
    ...
    public void quicksort() throws Exception{
      ...
      RandomAccessFile arq = new RandomAccessFile("matrıcula_quicksort.txt","rw");
      quicksortCabelo(0, n-1);
	  while ( i != (array.length-1) && j != (array.length-1) )
	  {
			while (array[j].getCorDoCabelo().compareTo(array[i].getCorDoCabelo()) == 0 ) 
			{
    ...
    * @param int dir fim do array a ser ordenado
    ...
    private void quicksortCabelo(int esq, int dir) {
        int i = esq, j = dir;
        StarWars pivo = array[(dir+esq)/2].clone();
