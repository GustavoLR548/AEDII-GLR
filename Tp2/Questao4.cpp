...
int main()
{
	...
	for (int i = 0; i <= n; i++)
	{
		...
		if (entrada->string[1] == '*' )
		{
			...
			{
				pos = charToInt(entrada->string);
				endereco = strstr(entrada->string,"/tmp");
				setPersonagem(endereco,Personagens);
				inserir(lista,Personagens,pos);
			}
			else if (entrada->string[0] == 'R')
			{
				pos = charToInt(entrada->string);
				Personagens = remover(lista,pos);
				printf("(R) %s\n", Personagens->nome->string);
			}
		}
		else if ( entrada->string[0] == 'I' )
		{
			if ( entrada->string[1] == 'I' )
			{
				endereco = strstr(entrada->string,"/tmp");
				setPersonagem(endereco,Personagens);
				inserirInicio(lista,Personagens);
			}
			else if (entrada->string[1] == 'F')
			{
				endereco = strstr(entrada->string,"/tmp");
				setPersonagem(endereco,Personagens);
				inserirFim(lista,Personagens);
		...
		else if ( entrada->string[0] == 'R' )
		{
			if ( entrada->string[1] == 'I' )
			{
				Personagens = removerInicio(lista);
				printf("(R) %s\n", Personagens->nome->string); 
			}
			else if (entrada->string [1] == 'F')
			{
				Personagens = removerFim(lista);
				printf("(R) %s\n", Personagens->nome->string);
			}
		}
